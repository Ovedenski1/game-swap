...
export default function ProfilePage() {
  ...
  const fetchProfile = useCallback(async (): Promise<UserProfile | null> => {
    ...
    return (profileData as UserProfile | null) ?? null;
  }, []);

  const fetchGames = useCallback(async (): Promise<Game[]> => {
    const myGames = await withTimeout(getMyGames(), 10000);
    return (myGames as Game[] | null | undefined) ?? [];
  }, []);

  const fetchCurrentRental = useCallback(async (userId: string): Promise<CurrentRental> => {
    ...
    const res = await withTimeout(rentalQuery, 10000);
    ...
    // Narrow shape safely (no `any`)
    const row = data as {
      id: string;
      status: string;
      due_date: string | null;
      rental_game: { title: string; platform: string; cover_url: string | null } | null;
    };
    ...
    return {
      ...
      rental_game: row.rental_game
        ? {
            title: String(row.rental_game.title),
            platform: String(row.rental_game.platform),
            cover_url: row.rental_game.cover_url ?? null,
          }
        : null,
    };
  }, []);
  ...
  const loadData = useCallback(
    async (mode: "initial" | "silent" = "initial") => {
      ...
      try {
        ...
        if (!hasProfileOnceRef.current) {
          setError(msg === "timeout" ? "Failed to load profile (timeout)." : "Failed to load profile.");
        } else {
          setSoftStatus(
            msg === "timeout"
        ...
        if (mode === "silent") {
          window.setTimeout(() => setSoftStatus(null), 1500);
        } else {
          setSoftStatus(null);
        }
      }
    ...
    return (
      <div className="max-w-[1500px] mx-auto px-3 sm:px-6 lg:px-4 py-6 sm:py-8">
        <header className="text-center">
          ...
          <p className="mt-3 text-xs sm:text-sm text-text-muted">Loading your profile…</p>
        </header>
    ...
    return (
      <div className="max-w-[1500px] mx-auto px-3 sm:px-6 lg:px-4 py-6 sm:py-8">
        <header className="text-center">
          ...
          <p className="mt-3 text-xs sm:text-sm text-text-muted">We couldn’t load your profile</p>
        </header>
  ...
  const preferredPlatforms = (profile?.preferences?.preferred_platforms || []) as GamePlatform[];
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-3 sm:px-6 lg:px-4 py-6 sm:py-8">
      ...
      <header className="text-center">
        ...
        <p className="mt-3 text-xs sm:text-sm text-text-muted">Manage your profile and swap preferences</p>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="flex items-center gap-4 sm:gap-6 mb-8">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-2xl font-bold mb-1 truncate">{profile?.full_name || "—"}</h2>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="pt-2 border-t border-border">
                    <h4 className="text-sm font-semibold mb-2">Platforms You Want to See</h4>

                    {preferredPlatforms.length === 0 ? (
                      <p className="text-xs text-white/60">All platforms (no filters selected).</p>
                    ) : (
                      <div className="flex flex-wrap gap-2 mt-1">
                        {preferredPlatforms.map((p) => {
                          const label = PLATFORM_LABELS[p] ?? String(p).toUpperCase();

                          let colorClass = "border-white/30 text-white/80";
                          if (p.startsWith("ps")) {
                            colorClass = "border-blue-400/70 text-blue-200";
                          } else if (p.startsWith("xbox")) {
                            colorClass = "border-emerald-400/70 text-emerald-200";
                          } else if (["switch", "wii", "wiiu", "ds", "3ds"].includes(p)) {
                ...
                <div>
                  ...
                  {games.length === 0 ? (
                    <p className="text-sm text-white/60">You haven’t added any games yet.</p>
                  ) : (
                      ...
                      {games.map((game) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-white/60 mb-1">{String(game.platform).toUpperCase()}</p>
                ...
                {currentRental && (
                  <div className="bg-black/30 border border-border rounded-xl p-4 mt-8">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold">{currentRental.rental_game?.title}</p>
                        <p className="text-xs text-white/60">{currentRental.rental_game?.platform}</p>
                        <p className="text-xs text-lime-400 mt-1 font-medium">🕹️ Status: Playing</p>

                        {currentRental.due_date &&
                          (() => {
                            const daysLeft = calculateDaysLeft(currentRental.due_date);
                            const colorClass =
                            ...
                            return (
                              <p className={`text-xs mt-1 font-semibold flex items-center gap-1 ${colorClass}`}>
                                ⏰{" "}
          ...
          <div className="bg-background/40 rounded-2xl border border-border shadow-[0_15px_40px_rgba(0,0,0,0.35)] p-6">
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-lg bg-white/5">
                ...
                <p className="text-sm text-white/80 break-words">{profile?.email || "—"}</p>
              </div>
